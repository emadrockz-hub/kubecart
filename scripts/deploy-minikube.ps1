...
Write-Host "`n[2/7] Ensuring namespace exists (kubecart)..."
kubectl get ns kubecart 1>$null 2>$null
if ($LASTEXITCODE -ne 0) {
  kubectl create ns kubecart | Out-Host
}

Write-Host "`n[3/7] Applying ConfigMaps..."
if (Test-Path ".\k8s\configmaps") {
  kubectl apply -n kubecart -f .\k8s\configmaps\ | Out-Host
} else {
...
if (Test-Path ".\k8s\secrets") {
  kubectl apply -n kubecart -f .\k8s\secrets\ | Out-Host
} else {
...
if (Test-Path ".\k8s\deployments") {
  kubectl apply -n kubecart -f .\k8s\deployments\ | Out-Host
} else {
...
if (Test-Path ".\k8s\services") {
  kubectl apply -n kubecart -f .\k8s\services\ | Out-Host
} else {
...
if (Test-Path ".\k8s\ingress") {
  kubectl apply -n kubecart -f .\k8s\ingress\ | Out-Host
} else {
  Write-Host "WARN: .\k8s\ingress not found"
}

Write-Host "`n== Waiting for pods =="
kubectl get pods -n kubecart | Out-Host
Write-Host "`nTip: If pods are not Running, use:"
Write-Host "  kubectl describe pod -n kubecart <pod-name>"
Write-Host "  kubectl logs -n kubecart <pod-name> --all-containers"
Write-Host "`n== Done =="
